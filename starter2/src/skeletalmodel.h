...
private:
    ...
    GLuint program;
    // helper function for drawing m_joints
    void jointDFS(const Camera& camera, Joint *joint);
    void boneDFS(const Camera& camera, Joint *joint);
    void bindDFS(Joint *joint, std::vector<Matrix4f> inverses);
    void updateDFS(Joint *joint);
    float findBoneLength(Joint *joint);
    Matrix4f rotationMatrix(Joint *joint);
};
